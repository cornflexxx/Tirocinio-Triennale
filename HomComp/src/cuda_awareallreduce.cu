...
int intra_node_reduce_scatter(float *d_sbuf, float *d_rbuf, size_t count,
  ...
  sum4arrays<<<blocks, threads>>>(d_rbuf + rank * per_rank_count,
                                  d_rbuf + per_rank_count * (rank + size - 1),
                                  d_rbuf + per_rank_count * (rank + size - 2),
                                  d_rbuf + per_rank_count * (rank + size - 3),
                                  recv_count);
...
int intra_node_allgather(float *d_rbuf, float *d_sbuf, size_t count,
  ...
  size_t send_count =
      (rank == size - 1) ? count - (per_rank_count)*rank : per_rank_count;
  for (int i = 0; i < size; i++) {
    recv_count = (i == size - 1) ? count - (per_rank_count)*i : per_rank_count;
    if (i != rank) {
      MPI_call_check(MPI_Isend(d_sbuf + rank * per_rank_count, send_count,
                               MPI_FLOAT, i, 0, local_comm, &send_reqs[i]));
      MPI_call_check(MPI_Irecv(d_rbuf + i * per_rank_count, recv_count,
                               MPI_FLOAT, i, 0, local_comm, &recv_reqs[i]));
    }
...
int mixed_compressed_allreduce(float *d_sbuf, float *d_rbuf, size_t count,
  ...
  // calcolo degli elementi per rank locale al nodo
  size_t per_rank_count = (local_rank == local_size - 1)
                              ? count - (count / local_size) * local_rank
                              : count / local_size;
  allreduce_ring_comprs_hom_sum_F(d_rbuf + local_rank * (count / local_size),
                                  d_rbuf + local_rank * (count / local_size),
