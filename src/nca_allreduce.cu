...
int cpuCopy_allreduce_ring_comprs_hom_sum(const float *d_sbuf, float *d_rbuf,
  ...
  for (k = 2; k < size; k++) {
    ...
    cmpSize2 = (size_t)count_;
    CUDA_CHECK(cudaMemcpy(d_tmpbuf, inbuf[inbi ^ 0x1], cmpSize2,
  ...
  cudaStreamSynchronize(quant_prediction_stream);
  CUDA_CHECK(
      cudaMemcpy(d_tmpbuf, inbuf[inbi], cmpSize2, cudaMemcpyHostToDevice));
...
int cpuCopy_allreduce_ring_comprs_hom_sum_seg(const float *d_sbuf,
  ...
  if (rank < split_rank) {
    ...
    if (rank == size - 1)
      block_count = count - block_offset_elements;
  }
